...
本脚本负责构建数据加载器、初始化 ImageOnlyModel、执行训练/验证循环，
并保存训练日志和验证集上表现最好的模型权重。
"""
...
def create_dataloaders() -> Dict[str, DataLoader]:
    ...
    # 如果可用 GPU，pin_memory 可以加快 CPU 到 GPU 的数据拷贝。
...
def main() -> None:
    """在训练集上训练 ImageOnlyModel，并在验证集上评估。"""
    ensure_output_dirs()

    # 将预训练权重缓存放在项目 outputs 目录下，避免散落到系统用户目录。
    ...
    model = ImageOnlyModel(num_classes=NUM_CLASSES, pretrained=True).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)

    log_path = LOG_DIR / "image_train_log.csv"
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # 只保存验证集 accuracy 最好的权重，方便后续测试集评估。
